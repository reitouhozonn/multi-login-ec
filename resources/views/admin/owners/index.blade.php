            ...
            <div class="container md:px-5 mx-auto">
                ...
                <table class="table-auto w-full text-left whitespace-no-wrap">
                    ...
                    <tr>
                        ...
                        <button onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id ]) }}'" class="flex ml-auto text-white bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded">編集</button>
                        </td>
                        <form id="delete_{{$owner->id}}" method="post" action="{{ route('admin.owners.destroy', ['owner' => $owner->id ]) }}">
                        @csrf
                        @method('delete')
                        <td class="md:px-4 py-3">
                            <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class="flex ml-auto text-white bg-red-400 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded">削除</a>
                        </td>
                        </form>
                    </tr>
                        @endforeach
